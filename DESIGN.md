...
4. **移动端手势交互优化（No Hover, Tap Active Feedback）**：
   - 屏退所有悬停相关的桥接线拉长、圆点放大、卡片高亮及边框变亮动画，防止移动端触控时产生动画残留与粘滞。
   - 桥接线长度在移动端保持平稳的固定 `18px` 宽度。
   - 增加点击激活状态（`:active`）的“浮起”反馈：当用户点击渡口时，节点整体上移 `4px`（即 `transform: translate(0, calc(-50% - 4px))`），提供轻快、即时的点按视觉反馈。
